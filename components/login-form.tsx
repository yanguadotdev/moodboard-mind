...
export function LoginForm({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  ...
  const handleGoogleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al intentar conectar con Google');
      setIsLoading(false);
    }
  };
    ...
    <div className={cn('flex flex-col gap-6', className)} {...props}>
      <Card className='border-zinc-800 bg-zinc-950 text-white'>
        <CardHeader className='text-center'>
          <CardTitle className='text-3xl font-mono tracking-tighter'>Bienvenido</CardTitle>
          <CardDescription className='text-zinc-400 font-mono'>Entra a tu jardín privado de ideas</CardDescription>
        </CardHeader>
        <CardContent>
