...
export function Roadmap() {
  ...
  return (
    <div className='w-full max-w-5xl mx-auto mt-10 mb-10 border-t border-zinc-900 pt-10'>
      <h3 className='font-mono text-xs uppercase tracking-[0.3em] text-zinc-500 mb-6'>Project Progress / Roadmap</h3>

      <Accordion type='multiple' defaultValue={['pending']} className='w-full space-y-4'>
        <AccordionItem value='pending' className='border-zinc-800 bg-zinc-900/10 rounded-xl px-4'>
          <AccordionTrigger className='hover:no-underline'>
            <div className='flex items-center gap-3'>
              <Badge variant='outline' className='border-amber-500/50 text-amber-500 bg-amber-500/5'>
                To-do ({pendingTasks.length})
              </Badge>
              <span className='text-sm font-medium text-zinc-300'>Core Features & UI</span>
            </div>
          </AccordionTrigger>
        ...
        <AccordionItem value='completed' className='border-zinc-800 bg-zinc-900/10 rounded-xl px-4'>
          <AccordionTrigger className='hover:no-underline'>
            <div className='flex items-center gap-3'>
              <Badge variant='outline' className='border-emerald-500/50 text-emerald-500 bg-emerald-500/5'>
                Done ({completedTasks.length})
              </Badge>
              <span className='text-sm font-medium text-zinc-300'>Infraestructura & Auth</span>
            </div>
          </AccordionTrigger>
          <AccordionContent className='space-y-4 pt-2 pb-4'>
            {completedTasks.map((task) => (
              ...
              >
                ...
                />
                <label htmlFor={task.id} className='text-sm font-mono text-zinc-400 line-through cursor-pointer'>
                  {task.label}
                </label>
              </div>
            ))}
